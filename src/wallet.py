...
class WalletManager:
    ...
    def __init__(self, private_key: str, rpc_url: str):
        ...
        self.networks = {
            ...
            # Added cross-chain networks for PHRS ↔ Unichain swaps
            "pharos": {
    ...
    def switch_network(self, token_type: str):
        """Switch active network based on token type or chain uid"""
        token_lc = (token_type or "").lower()
        if token_lc in ["stt", "somnia"]:
            target_network = "somnia"
        elif token_lc in ["phrs", "pharos"]:
            target_network = "pharos"
        elif token_lc in ["eth", "unichain"]:
            target_network = "unichain"
        else:
    ...
    def execute_swap_transaction(self, swap_data: Dict[str, Any], token_in: str = "plume", wait_for_receipt: bool = True) -> str:
        ...
        try:
            ...
            # Prefer RPC URL from API response if provided (cross-chain safe)
            api_rpc_url = swap_data.get('rpc_url') or swap_data.get('rest_url')
            temp_w3 = None
            if api_rpc_url:
                try:
                    temp_w3 = Web3(Web3.HTTPProvider(api_rpc_url))
                except Exception:
                    temp_w3 = None

            # Switch to appropriate network based on input token as fallback
            ...
            w3_for_tx = temp_w3 or self.w3
            # Prefer chainId from API msg if present
            msg_chain_id = None
            try:
                if 'chainId' in msg:
                    raw_cid = msg['chainId']
                    msg_chain_id = int(raw_cid, 0) if isinstance(raw_cid, str) and raw_cid.startswith('0x') else int(raw_cid)
            except Exception:
                msg_chain_id = None
            ...
            try:
                estimated_gas = w3_for_tx.eth.estimate_gas({
            ...
            # Optionally wait for confirmation on the same provider
            if wait_for_receipt:
