...
class AdaptiveConfigManager:
    ...
    def prompt_user_for_configuration(self, token_label: Optional[str] = None) -> Tuple[AdaptiveConfiguration, bool]:
        """
        ...
        """
        try:
            token_ctx = (token_label or 'tokens').upper()
            ...
            print(f"\n{Fore.GREEN}✓ Amount: {initial_amount}{Style.RESET_ALL}")
            ...
            if mode == AdaptiveMode.FIXED:
                ...
                print(f"{Fore.WHITE}  - Bot will always use {initial_amount} {token_ctx}{Style.RESET_ALL}")
    ...
    def _configure_adaptive_settings(self, initial_amount: float) -> Tuple[AdaptiveConfiguration, bool]:
        ...
        try:
            ...
            print(f"├─ Range: {initial_amount} - 1.0")
            ...
            print(f"- Start with {initial_amount}")
    ...
    def prompt_use_saved_configuration(self) -> bool:
        ...
        try:
            ...
            print(f"├─ Initial Amount: {saved_config.initial_amount}")
            print(f"├─ Current Amount: {saved_config.current_amount}")
            
            if saved_config.optimal_amount:
                savings = ((1.0 - saved_config.optimal_amount) / 1.0) * 100
                print(f"├─ Optimal Found: {saved_config.optimal_amount} ({savings:.1f}% savings)")
            
            print(f"└─ Total Optimized: {saved_config.tokens_saved:.3f}")
