...
class Colors:
    ...
    def colorize_brackets(text):
        ...
        color_map = {
            ...
            'STATS': '\033[95m',  # Purple for statistics
            'WALLET': Colors.BLUE,
            'MODE': Colors.BLUE,
            ...
            'FAILED': Colors.RED,
            'IBC': '\033[38;5;208m',  # Soft orange for cross-chain
            'PROGRESS': '\033[90m'
        }
...
class CleanUI:
    ...
    def display_swap_step(self, swap_num, direction, message):
        """Dynamic status updates during swap execution"""
        print(Colors.colorize_brackets(f"[SWAP #{swap_num}] {direction} - {message}"))
    ...
    def display_swap_failed(self, error_msg):
        ...
        print(Colors.colorize_brackets(f"[FAILED] {error_msg}"))

    def display_mode_banner(self, continuous=False, session_total=0, grand_total=0):
        label = "Continuous swapping active (Stop with Ctrl+C)" if continuous else "Multiple swaps mode"
        print(Colors.colorize_brackets(f"[MODE] {label}"))
        print(Colors.colorize_brackets(f"[PROGRESS] Session: {session_total} | Total: {grand_total}"))
